...
const TipoActividadPage = () => {
         ...
         confirmPopup({
            ...
            accept: async () => {
                try {
                    ...
                    const response:any = await TipoActividadService.deleteTipoActividad(data.id);
                    ...
                    showSuccess('TipoActividad eliminado correctamente');
    ...
    const handleSave = async (e:any) => {
        ...
        if (!data.id) {
            try {
               ...
               const response:any = await TipoActividadService.createTipoActividad(contexto);
               const createdTipoActividad = await response.data;

               updateRows({ ...createdTipoActividad, keyString: data.keyString }, index);
               cleanRowsLoading(data); 
               cleanRowsEditing(data);
               showSuccess('TipoActividad creada correctamente');
            ...
            try {
                ...
                const response = await TipoActividadService.updateTipoActividad(formulario.id,contexto);
                const updatedTipoActividad = await response.data;
             
                updateRows({ ...updatedTipoActividad, keyString: data.keyString }, index);
                cleanRowsLoading(data);
                cleanRowsEditing(data);
                showSuccess('Tipo de actividad actualizada correctamente');
    ...
    const updateRows = (data:any, index:any, isDelete:boolean=false) => {
        ...
        setTipoActividades((prev:any) => {

            let updatedTipoActividades = [...prev];
            if(isDelete) {
                updatedTipoActividades = updatedTipoActividades.filter((tipoActividad:any, idx:any) => index !== idx)
            } else {
                updatedTipoActividades[index] = {
                    ...data,
                };
            }
            return updatedTipoActividades;
        });
    }
    ...
    const onRowEditCancel = (e:any) => {
        ...
        if(!data.id) {
            setTipoActividades((prev:any) => prev.filter((item:any) => item.keyString !== data.keyString)); 
        }
    };
