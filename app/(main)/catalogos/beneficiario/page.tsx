...
const BeneficiarioPage = () => {
    ...
    const onAgregar = () => {
        ...
        setBeneficiarioes(prev => [nuevo, ...prev]); 
         ...
         confirmPopup({
            ...
            accept: async () => {
                try {
                    ...
                    const response:any = await BeneficiarioService.deleteBeneficiario(data.id);
                    const createdBeneficiario = await response.data;

                    updateRows({ ...createdBeneficiario, keyString: data.keyString }, index, true);
                    cleanRowsDeleting(data);
                    showSuccess('Beneficiario eliminado correctamente');
    ...
    const handleSave = async (e:any) => {
        ...
        if (!data.id) {
            try {
               ...
               const response:any = await BeneficiarioService.createBeneficiario(contexto);
               const createdBeneficiario = await response.data;

               updateRows({ ...createdBeneficiario, keyString: data.keyString }, index);
               cleanRowsLoading(data); 
               cleanRowsEditing(data);
               showSuccess('Beneficiario creada correctamente');

            } catch (error:any) {

                showError(error.message || 'Error al crear la Beneficiario');
                return;
            }
            ...
            try {
                ...
                const response = await BeneficiarioService.updateBeneficiario(formulario.id,contexto);
                const updatedBeneficiario = await response.data;
             
                updateRows({ ...updatedBeneficiario, keyString: data.keyString }, index);
                cleanRowsLoading(data);
                cleanRowsEditing(data);
                showSuccess('Beneficiario actualizada correctamente');
                
            } catch( error:any) {

                showError('Error', error.message || 'Error al actualizar la Beneficiario');
                return;
            }
        }
    ...
    const updateRows = (data:any, index:any, isDelete:boolean=false) => {
        ...
        setBeneficiarioes((prev:any) => {

            let updatedBeneficiarioes = [...prev];
            if(isDelete) {
                updatedBeneficiarioes = updatedBeneficiarioes.filter((Beneficiario:any, idx:any) => index !== idx)
            } else {
                updatedBeneficiarioes[index] = {
                    ...data,
                };
            }
            return updatedBeneficiarioes;
        });
    }
    ...
    const onRowEditCancel = (e:any) => {
        ...
        if(!data.id) {
            setBeneficiarioes((prev:any) => prev.filter((item:any) => item.keyString !== data.keyString)); 
        }
    };
